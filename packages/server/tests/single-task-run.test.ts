...
test('orphan recovery returns 500 and does not insert/broadcast events if repo.update fails', async () => {
  const db = new Database(':memory:');
  migrateSqliteDatabase(db);
  const realRepo = new SqliteTaskRepository(db);
  ...
  const fakeAgentManager = {
    runningTaskIds: new Set<string>(),
    resetEvents: (taskId: string) => { resetEventsCalls.push(taskId); },
    isRunning: (_taskId: string) => false,
    startAgent: (_task: Task, _onStatusChange: (status: AgentStatus) => void | Promise<void>) => {
      assert.fail('startAgent should not be called for orphan recovery');
    },
    stopAgent: async (_taskId: string) => false,
    getEvents: async (_taskId: string) => [],
    sendMessage: async (_taskId: string, _message: string) => false,
  ...
  const failingRepo = new Proxy(realRepo, {
    get(target, prop, receiver) {
      if (prop === 'update') {
        return async (_id: string, updates: Partial<Task>) => {
          if (updates.agentStatus === 'failed') {
            return undefined;
          }
          return target.update(_id, updates);
        };
      }
      if (prop === 'insertEvent') {
        return async () => {
          assert.fail('insertEvent must not be called when orphan recovery update fails');
        };
      }
      return Reflect.get(target, prop, receiver);
    },
  ...
  try {
    const task = baseTask({ id: 'orphan-update-fail', agentStatus: 'executing' });
    await realRepo.create(task);
    ...
    assert.equal(status, 500, `expected 500, got ${status}: ${JSON.stringify(body)}`);
    assert.ok(body.error);
    assert.deepStrictEqual(resetEventsCalls, [], 'resetEvents must not be called during failed orphan recovery');

    const events = await realRepo.getEventsByTaskId(task.id);
    assert.equal(events.length, 0, 'no events should be persisted when orphan recovery update fails');
  } finally {
    db.close();
  }
});

test('getPendingRunsexcludes orphaned planning tasks from startup auto-run', async () => {
  const db = new Database(':memory:');
  migrateSqliteDatabase(db);
  const repo = new SqliteTaskRepository(db);

  try {
    const task = baseTask({ id: 'pending-planning', agentStatus: 'planning' });
    await repo.create(task);
    await repo.requestRun(task.id, Date.now() - 60_000);

    const pending = await repo.getPendingRuns(Date.now());
    assert.equal(pending.length, 0, 'planning orphans must not be eligible for startup dispatch');
  } finally {
    db.close();
  }
});
